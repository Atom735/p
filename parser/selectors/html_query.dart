...
import 'base.dart' show rws;

Element? myQuerySelector(Element node, String selector) {
  if (selector.isEmpty) {
    return node;
  }
  if (selector == '+') {
  ...
  return MySelectorEvaluator()
      .querySelector(node, _parseSelectorList(selector));
}

List<Element> myQuerySelectorAll(Element node, String selector) {
  if (selector.isEmpty) {
...
class MySelectorEvaluator extends SelectorEvaluator {
  ...
  int _countExpressionList(List<Expression> list) {
    final first = list[0];
    var sum = 0;
    var modulus = 1;
    ...
    list.forEach((Expression item) {
      sum += ((item as NumberTerm).value as num).toInt();
    });
    return sum * modulus;
  }

  MapEntry<int?, int>? _parseNthExpressions(List<Expression> exprs) {
    int? A;
    var B = 0;
    ...
    if (exprs.isNotEmpty) {
      if (exprs.length == 1 && (exprs[0] is LiteralTerm)) {
        ...
        if (literal is NumberTerm) {
          ...
          final value = literal.value.toString();
          if (value == 'even') {
            A = 2;
            B = 1;
          } else if (value == 'odd') {
      ...
      final bTerms = <Expression>[];
      final aTerms = <Expression>[];
      final nIndex = exprs.indexWhere((expr) {
  ...
  bool visitAttributeSelector(AttributeSelector selector) {
    final attr = selector.name.toLowerCase();
    if (attr == 'text') {
      final select = '${selector.value}';
      return rws(_element!.text) == select;
    }
    if (attr == 'regexp') {
      final select = RegExp('${selector.value}');
      return select.hasMatch(_element!.text);
    }
    // Match name first
    final value = _element!.attributes[attr];
